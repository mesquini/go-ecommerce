...
class ProductsRepository implements IProductsRepository {
  ...
  public async findAllById(productsId: IFindProducts[]): Promise<Product[]> {
    const idList = productsId.map(product => product.id);
    const products = await this.ormRepository.find({ id: In(idList) });
  ...
  public async updateQuantity(
    ...
    const newProducts = productsData.map(productData => {
      ...
      if (!productFind) throw new AppError('Product not find');
